...
void start_fcfs(vector<vector<int> > process_p, queue<int> ready_queue, vector<int> process_burst_index);
...
int main(int argc, char const* argv[]){
    ...
    /* loop through the processes and put them into ready queue */
    for(int i=0; i < process_p.size(); i++) {
        ready_queue_FCFS.push(i);
    }
    /* FCFS runs now */
    start_fcfs(process_p, ready_queue_FCFS, process_burst_index);
...
void start_fcfs(vector<vector<int> > process_p, queue<int> ready_queue, vector<int> process_burst_index)
{
    ...
    /* while ready queue is empty, set process index to front ready queue */
    while(!ready_queue.empty()){
        ...
        /* if first element in ioreturn is less than time, push ready queue
        to ioreturn second. arrival time ioreturn second is equal to ioreturn first */

            for(int i=0; i < process_p.size(); i++){
                if(io_return[i].first <= time){
                    ready_queue.push(io_return[i].second);
